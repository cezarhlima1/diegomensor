...
export async function GET() {
  ...
  const { data, error } = await admin.from("profiles").select("id, nome, email, crm_access, crm_is_admin, crm_permissions, created_at").eq("crm_access", true).order("created_at");
  if (error) return NextResponse.json({ error: "database-unavailable" }, { status: 503 });
  return NextResponse.json({ users: (data || []).map((profile) => ({ id: profile.id, name: profile.nome || "", email: profile.email, isAdmin: profile.crm_is_admin, permissions: profile.crm_permissions || [] })) });
}

export async function POST(request: Request) {
  const caller = await requireCrmAdmin();
  if (!caller) return NextResponse.json({ error: "forbidden" }, { status: 403 });
  let body: { name?: string; email?: string; password?: string; permissions?: unknown; isAdmin?: boolean };
  try { body = await request.json(); } catch { return NextResponse.json({ error: "invalid-json" }, { status: 400 }); }
  const name = String(body.name || "").trim();
  const email = String(body.email || "").trim().toLowerCase();
  const password = String(body.password || "");
  const permissions = validPermissions(body.permissions);
  if (!name || !/^\S+@\S+\.\S+$/.test(email) || password.length < 8 || (!body.isAdmin && !permissions.length)) return NextResponse.json({ error: "invalid-payload" }, { status: 400 });
  const admin = createSupabaseAdminClient();
  ...
  const { error: profileError } = await admin.from("profiles").update({ nome: name, email, crm_access: true, crm_is_admin: Boolean(body.isAdmin), crm_permissions: effectivePermissions }).eq("id", created.user.id);
  if (profileError) {
    await admin.auth.admin.deleteUser(created.user.id);
    return NextResponse.json({ error: "profile-update-failed" }, { status: 503 });
  }
  return NextResponse.json({ ok: true, user: { id: created.user.id, name, email, isAdmin: Boolean(body.isAdmin), permissions: effectivePermissions } }, { status: 201 });
}

export async function PATCH(request: Request) {
  if (!await requireCrmAdmin()) return NextResponse.json({ error: "forbidden" }, { status: 403 });
  let body: { id?: string; name?: string; email?: string; password?: string; permissions?: unknown; isAdmin?: boolean };
  try { body = await request.json(); } catch { return NextResponse.json({ error: "invalid-json" }, { status: 400 }); }
  ...
  const password = String(body.password || "");
  const permissions = validPermissions(body.permissions);
  if (!id || !name || !/^\S+@\S+\.\S+$/.test(email) || (password && password.length < 8) || (!body.isAdmin && !permissions.length)) return NextResponse.json({ error: "invalid-payload" }, { status: 400 });
  const admin = createSupabaseAdminClient();
  ...
  const effectivePermissions = body.isAdmin ? [...CRM_TABS] : permissions;
  const { error } = await admin.from("profiles").update({ nome: name, email, crm_access: true, crm_is_admin: Boolean(body.isAdmin), crm_permissions: effectivePermissions }).eq("id", id);
