...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      traffic: trafficResult.rows.map((row) => ({ id: row.id, month: row.month, date: row.campaign_date ? String(row.campaign_date).slice(0, 10) : undefined, status: row.status, campaign: row.name, product: row.product, investment: Number(row.investment), clicks: row.clicks, pageViews: row.page_views, checkouts: row.checkouts, sales: row.sales, revenue: Number(row.gross_revenue), netRevenue: row.net_revenue == null ? undefined : Number(row.net_revenue) })),
