...
function Dashboard({ leads, stats, openPipeline }: { leads: Lead[]; stats: { total: number; meetings: number; proposals: number; closed: number; openValue: number; wonValue: number; conversion: number; hot: number }; openPipeline: () => void }) {
  const sources = Array.from(new Set(leads.map((lead) => lead.source))).map((source) => {
    ...
    return { source, leads: sourceLeads.length, meetings, closed, share: leads.length ? sourceLeads.length / leads.length * 100 : 0, conversion: sourceLeads.length ? closed / sourceLeads.length * 100 : 0, value: sourceLeads.reduce((sum, lead) => sum + lead.value, 0) };
