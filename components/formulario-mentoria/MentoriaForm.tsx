...
export default function MentoriaForm() {
  ...
  useEffect(() => { const audio = audioRef.current; if (!audio) return; audio.volume = .07; void audio.play().then(() => setPlaying(true)).catch(() => setPlaying(false)); }, []);
  useEffect(() => { if (!started || done) return; setTyping(true); const timer = window.setTimeout(() => { setTyping(false); window.setTimeout(() => inputRef.current?.focus(), 40); }, index ? 360 : 650); return () => window.clearTimeout(timer); }, [index, started, done]);
  ...
  function start() { const audio = audioRef.current; if (audio) { audio.volume = .07; void audio.play().then(() => setPlaying(true)).catch(() => setPlaying(false)); } setStarted(true); }
  ...
  if (!started) return <>{audio}<Intro start={start} />{music}</>;
...
function Intro({ start }: { start: () => void }) { return <main className={styles.intro}><div className={styles.introPattern} /><section><div className={styles.mentorBrand}><b>@diegomensor</b><i>|</i><span>Mentor de donos de oficina</span></div><p className={styles.kicker}>Aplicação para mentoria</p><h1>Vamos conversar sobre o <em>próximo nível da tua oficina?</em></h1><p className={styles.introLead}>Preparei uma conversa rápida para entender teu momento, teus desafios e descobrir se a mentoria faz sentido para ti hoje.</p><div className={styles.introMeta}><span><b>19</b> perguntas</span><span><b>5</b> minutos</span><span><b>100%</b> confidencial</span></div><button onClick={start}>Começar conversa <span>→</span></button><small>Responda com calma e sinceridade.</small></section></main>; }
